...
from fedml.cross_silo.client.message_define import MyMessage
from fedml.cross_silo.client.utils import convert_model_params_from_ddp, convert_model_params_to_ddp
...
class ClientMasterManager(FedMLCommManager):
    ...
    def handle_message_receive_model_from_server(self, msg_params):
        ...
        logging.info("current round index {}, total rounds {}".format(
        ...
        if self.round_idx < self.num_rounds:
            ...
            self.send_client_status(
                0, ClientMasterManager.RUN_FINISHED_STATUS_FLAG)
    ...
    def send_model_to_server(self, receive_id, weights, local_sample_num):
        ...
        with self.logger.communication(target_id=0) as c:
            message = Message(
                MyMessage.MSG_TYPE_C2S_SEND_MODEL_TO_SERVER, self.client_real_id, receive_id,)
            message.add_params(MyMessage.MSG_ARG_KEY_MODEL_PARAMS, weights)
            message.add_params(
                MyMessage.MSG_ARG_KEY_NUM_SAMPLES, local_sample_num)
    ...
    def send_client_status(self, receive_id, status=ONLINE_STATUS_FLAG):
        ...
        if hasattr(self.args, "using_mlops") and self.args.using_mlops and \
            ...
            mlops.log_server_payload(
                self.args.run_id, self.client_real_id, json.dumps(message.get_params()))
        else:
            self.send_message(message)
    ...
    def sync_process_group(self, round_idx, model_params=None, client_index=None, src=0):
        ...
        logging.info("round number %d broadcast to process group" %
                     round_number[0])
